 ...
 * validate-process.ts. It does not check cross-file references: it does not
 * confirm that process_nodes exist in repair-management.yaml, that
 * business_rules exist in business-rules.yaml, or that source_process.version
 * matches the real Business Process. That is a later evolution.
 *
...
function findDuplicateIds(features: Feature[]): string[] {
...
function main(): void {
  ...
  const data = parse(readFileSync(FEATURES_FILE, "utf8")) as FeatureFile;
